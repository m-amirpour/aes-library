...
inline uint8_t u8(std::byte b) {
    return std::to_integer<uint8_t>(b);
}

inline uint32_t load_be(const std::byte* p) {
    return (uint32_t(u8(p[0])) << 24) | (uint32_t(u8(p[1])) << 16) | (uint32_t(u8(p[2])) << 8) |
           uint32_t(u8(p[3]));
}

inline void store_be(std::byte* p, uint32_t w) {
    p[0] = std::byte((w >> 24) & 0xff);
    p[1] = std::byte((w >> 16) & 0xff);
    p[2] = std::byte((w >> 8) & 0xff);
    p[3] = std::byte(w & 0xff);
}
...
void ctr_xor(const std::byte* rk, const std::byte* ctr_in, const std::byte* in, std::byte* out,
             std::size_t len) {
    std::byte ctr[16];
