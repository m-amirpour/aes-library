...
EncryptResult encrypt(const Key& key, const std::vector<std::byte>& pt) {
    if (pt.size() > MAX_BYTES)
        throw CryptoError("plaintext too large for single nonce");
    ...
    if (!pt.empty()) {
        auto be = detail::get_backend();
        be.cx(key.expanded_key(), r.nonce.data(),
...
{
    if (ct.size() > MAX_BYTES)
        throw CryptoError("ciphertext too large");

    std::vector<std::byte> pt(ct.size());
    if (!ct.empty()) {
        auto be = detail::get_backend();
        be.cx(key.expanded_key(), nonce.data(),
              ct.data(), pt.data(), ct.size());
    }
    return pt;
}

void encrypt_to_file(const Key& key,
    const std::vector<std::byte>& pt,
...
std::vector<std::byte> decrypt_from_file(const Key& key,
    const std::string& path)
{
