...
const char* aes_path_name(AesPath p) noexcept {
    switch (p) {
        case AesPath::Software:        return "Software (portable)";
        case AesPath::Hardware_AESNI:  return "Hardware (AES-NI)";
        case AesPath::Hardware_ARM_CE: return "Hardware (ARM CE)";
    }
    return "Unknown";
}

AesPath active_aes_path() noexcept {
    return t_override.value_or(cached());
}
...
// Internal: used by aes.cpp and key.cpp to get the active backend.
namespace detail {
